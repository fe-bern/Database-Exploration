...
engine_mysql = create_engine(conns, encoding="latin1", echo=True)
...
def load_smaller_to_postgres(**context):
    ...
    ethereum_final.to_sql('table_ethereum', con=engine_psql, if_exists='replace', method='multi')
...
#create extract task
extract_from_mysql = PythonOperator(task_id='extract_from_mysql', python_callable=extract_from_mysql, dag=dag)

#create transform task
transform_big_to_smaller = PythonOperator(task_id='transform_big_to_smaller', python_callable=transform_big_to_smaller,
                           provide_context=True, dag=dag)

#create load task
load_smaller_to_postgres = PythonOperator(task_id='load_smaller_to_postgres', python_callable=load_smaller_to_postgres,
                      provide_context=True, dag=dag)

#create transform2 task
transform_only_stats = PythonOperator(task_id='transform_only_stats', python_callable=transform_only_stats,
                           provide_context=True, dag=dag)

#create load2 task
load_stats_to_mongodb = PythonOperator(task_id='load_stats_to_mongodb', python_callable=load_stats_to_mongodb,
                      provide_context=True, dag=dag)
